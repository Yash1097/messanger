...
function make_chat_dialog_box(to_user_id, to_user_name)
	{
        ...
        document.getElementById('chatbg').style.height = heights*0.98 + "px";
        createinputarea(to_user_id);
        return modal_content;
	}
function createinputarea(to_user_id){
    var inputstyle = '<div><div class="input-group"><input type="text" id="chat_message_'+to_user_id+'" class="text-dark form-control chat_message" placeholder="Type something here"><div class="input-group-append"><button type="button" name="send_chat" id="'+to_user_id+'" class="send_chat"><i class="fa fa-paper-plane"></i></button></div></div></div></div></div>';
...
function scrolld () {
	...
	$(document).on('click', '.nav-link', function(){
        ...
        document.getElementById('chatwith').innerHTML = '<div class="row"><img id="pimg" class="ml-3 rounded-circle" '+getimg(to_user_id)+'" width="5%"><div class="text-light ml-2"><h2><b>'+to_user_name+'</b></h2></div></div>';
        document.getElementById('v-pills-tabContent').innerHTML = make_chat_dialog_box(to_user_id, to_user_name);
        var heights = window.innerHeight;
        document.getElementById('v-pills-tabContent').style.height = heights*0.89 + "px";
    ...
    function getimg(to_user_id){
        $.ajax({
			url:"getimg.php",
			method:"POST",
			data:{to_user_id:to_user_id},
			success:function(data){
                document.getElementById('pimg').src = 'upload/'+data;
			}
		})
    }
